...
#
# This module generates dream-inspired multimodal outputs — combining
# text, audio, and visuals using large generative models.
#
# It integrates:
...
#
# All assets and metadata are saved under BASE/results/ for inspection.
...
# Output directory (project-root style)
# If you run from the repo root: BASE = <repo root>
# If you run from notebooks/: BASE = notebooks/
BASE = Path(".").resolve()
...
# ---------------------------------------------------------------------
# Core image generation + moodboard
# ---------------------------------------------------------------------
def text_to_image(
    prompt: str,
...
MOTIF_LEXICON = {
    "forest", "tree", "river", "sea", "ocean", "wave", "water", "rain", "storm", "cloud",
    "sky", "moon", "sun", "star", "night", "light", "shadow",
    "mirror", "glass", "window", "door", "stairs", "bridge", "room", "house", "city",
    "desert", "mountain", "valley", "garden", "flower",
...
def multimodal_with_extras(
    ...
    """
    Extended multimodal generation:
    - main SD-Turbo image + MusicGen audio
    - optional moodboard
    - optional motif graph
    - structured JSON report
    """
    report, base_img = multimodal_from_text(
        user_text,
...
# ---------------------------------------------------------------------
# Public entrypoint used by Gradio UI / evaluation notebook
# ---------------------------------------------------------------------
def run_all_with_audio(
    ...
    """
    - If 'prefer_audio' is True and an audio file is provided, use ASR transcript.
    - Otherwise, use the text box input.
    Returns:
